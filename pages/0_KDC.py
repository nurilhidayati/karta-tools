...
def load_data():
    ...
    # Ensure timeseries is sorted by date
    ts = ts.sort_values("install_date")
    
    # Sort alerts by timestamp (most recent first)
    alerts = alerts.sort_values("timestamp", ascending=False)
    
    # Make datetime columns timezone-naive for consistent comparison
    devices["last_seen"] = pd.to_datetime(devices["last_seen"]).dt.tz_localize(None)
    devices["install_date"] = pd.to_datetime(devices["install_date"]).dt.tz_localize(None)
    ts["install_date"] = pd.to_datetime(ts["install_date"]).dt.tz_localize(None)
    alerts["timestamp"] = pd.to_datetime(alerts["timestamp"]).dt.tz_localize(None)
...
hours = st.sidebar.slider("Last seen within (hours)", 0, 72, 72)
st.sidebar.caption("💡 Set to 0 to show all devices regardless of last seen time")
...
c1.metric("Active Devices", total, delta=f"of {total_devices} total")
...
with right:
    ...
    alerts_display = alerts.head(18).copy()
    alerts_display["timestamp"] = alerts_display["timestamp"].dt.strftime("%m/%d %H:%M")
    st.dataframe(alerts_display, height=260, use_container_width=True)
...
# Format device dataframe for better display
df_display = df.sort_values("last_seen", ascending=False)[
...
df_display["last_seen"] = df_display["last_seen"].dt.strftime("%m/%d %H:%M")
