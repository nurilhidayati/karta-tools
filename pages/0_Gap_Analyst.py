...
def generate_ai_recommendations(analysis_results):
    ...
    if 'area_types' in analysis_results and analysis_results['area_types']:
        ...
        if most_common_areas:
            ...
            if top_area_type in ['High Security', 'Military/Security']:
                recommendations.append({
                    'type': 'SECURITY',
                    'priority': 'HIGH',
                    'message': f"⚠️ High security risk: {top_area_count} military/government areas affected. Consider alternative routing through public roads."
                })
            elif top_area_type == 'Educational':
                recommendations.append({
                    'type': 'SAFETY',
                    'priority': 'HIGH',
                    'message': f"🏫 School safety concern: {top_area_count} educational facilities affected. Ensure child safety protocols during operations."
                })
            elif top_area_type == 'Healthcare':
                recommendations.append({
                    'type': 'EMERGENCY',
                    'priority': 'CRITICAL',
                    'message': f"🏥 Healthcare access critical: {top_area_count} medical facilities affected. Maintain emergency vehicle access at all times."
                })
    ...
    if 'geographic_patterns' in analysis_results:
        geo_data = analysis_results['geographic_patterns']
        hotspots = geo_data.get('hotspot_analysis', {})
        
        if hotspots.get('hotspot_count', 0) > 3:
            recommendations.append({
                'type': 'LOGISTICS',
                'priority': 'MEDIUM',
                'message': f"📍 {hotspots['hotspot_count']} high-density restriction zones detected. Consider zone-based operational planning."
            })
    
    # Road restriction recommendations
    if 'road_restrictions' in analysis_results:
        road_data = analysis_results['road_restrictions']
        if road_data.get('total_roads', 0) > 10:
            recommendations.append({
                'type': 'ACCESS',
                'priority': 'MEDIUM',
                'message': f"🛣️ {road_data['total_roads']} restricted roads identified. Verify access permissions and alternative routes."
            })
    ...
    if not recommendations:
        recommendations.append({
            'type': 'GENERAL',
            'priority': 'LOW',
            'message': "✅ Analysis shows manageable restrictions. Proceed with standard operational protocols."
        })
...
if uploaded_file is not None:
    if st.button("🚀 **Run Complete Gap Analysis**", type="primary"):
        ...
        try:
            ...
            current_status.info("🤖 Bonus: Running AI analysis of OSM features...")
